...
export default function PodcastGenerator({
  ...
  const saveAudio = async () => {
    if (!audioUrl || isSaved) return
    try {
      const urlObj = new URL(audioUrl, "http://localhost")
      const relPath = urlObj.pathname
      
      const res = await api.post<{audio_url: string}>("/generate/podcast/save", { audio_url: relPath })
      if (res.audio_url) {
        const baseUrl = import.meta.env.VITE_API_URL || "http://localhost:8000"
        setAudioUrl(`${baseUrl}${res.audio_url}`)
        setIsSaved(true)
      }
    } catch (err) {
    ...
    <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-colors cursor-pointer"
              style={{
            ...
            <button
              onClick={saveAudio}
              disabled={isSaved}
              className="flex-1 py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              style={{
                background: isSaved ? "var(--color-success-bg)" : "var(--color-accent-bg)",
                color: isSaved ? "var(--color-success)" : "var(--color-accent-primary)",
              }}
            >
              <CheckCircle className="w-4 h-4" /> {isSaved ? "Saved" : "Keep on Server"}
            </button>
          </div>
        </div>
      )}
      ...
      {script.length > 0 && (
        <div>
          ...
          <div
            className="max-h-48 overflow-y-auto p-3 rounded-xl text-sm space-y-2"
            style={{ background: "var(--color-bg-tertiary)" }}
          >
            ...
            {script.length > 10 && (
              <p
                ...
                style={{ color: "var(--color-text-muted)" }}
              >
                ... and {script.length - 10} more lines
              </p>
            )}
          </div>
        </div>
